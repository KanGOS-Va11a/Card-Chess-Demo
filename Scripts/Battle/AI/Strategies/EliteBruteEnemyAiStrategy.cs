...
public sealed class EliteBruteEnemyAiStrategy : IEnemyAiStrategy
{
    ...
    public EnemyAiDecision Decide(EnemyAiContext context)
    {
        ...
        int effectiveMove = context.SelfState.MovePointsPerTurn;
        if (context.SelfState.MaxHp > 0 && context.SelfState.CurrentHp * 2 <= context.SelfState.MaxHp)
        {
            effectiveMove += 1;
        }
        ...
        Vector2I? nextCell = context.Pathfinder
            .FindReachableCells(context.Self.ObjectId, context.Self.Cell, effectiveMove)
