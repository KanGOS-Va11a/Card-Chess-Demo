...
public partial class PartnerManager : Node
{
    ...
    [Export] public string DefaultPartnerScenePath = "res://legacy/2026-03-30-script-audit/Scene/Partner.tscn";
    ...
    public override void _Ready()
    {
        if (PartnerScene == null && !string.IsNullOrWhiteSpace(DefaultPartnerScenePath))
        {
            ...
            {
                GD.PushWarning($"PartnerManager: 浼欎即鍦烘櫙鍔犺浇澶辫触锛岃矾寰?'{normalizedPath}'銆?);
            }
        }
    }
    ...
    private void EnsureRuntimePartner(Node currentScene, Player player)
    {
        ...
        if (PartnerScene == null)
        {
            GD.PushWarning("PartnerManager: PartnerScene 鏈厤缃紝鏃犳硶鑷姩鐢熸垚浼欎即銆?);
            return;
        }
