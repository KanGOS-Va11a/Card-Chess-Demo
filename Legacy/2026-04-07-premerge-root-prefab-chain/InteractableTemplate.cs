...
public abstract partial class InteractableTemplate : StaticBody2D, IInteractable
{
	...
	protected ulong _nextAvailableTimeMs;
	...
	private void ApplyFlowProgressState()
	{
		...
		{
			session.SetFlag(new StringName($"flow.completed.{CompletedFlowNodeId.Trim()}"), true);
		}

		if (!string.IsNullOrWhiteSpace(NextActiveFlowNodeId))
		{
			session.SetFlag(new StringName("flow.active_node"), NextActiveFlowNodeId.Trim());
		}
	}
}
